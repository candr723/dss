...
function generateAHPTable() {
  ...
  table += "</table>";
  table += `<button type="button" onclick="calculateAHP(${criteria})" class="form-button-2">Hitung AHP</button>`;
...
function calculateAHP(criteria) {
  ...
  // Tampilkan hasil
  let result =
    "<h3>Hasil AHP</h3><table border='1'><tr><th>Kriteria</th><th>Bobot</th></tr>";
  ...
  result += `<p><strong>Consistency Ratio (CR):</strong> ${consistencyRatio.toFixed(
    4
  )}</p>`;
  if (consistencyRatio > 0.1) {
    result +=
      "<p><span style='color: red;'>CR lebih dari 0.1, matriks tidak konsisten.</span></p>";
  } else {
    result +=
      "<p><span style='color: green;'>CR kurang dari 0.1, matriks konsisten.</span></p>";
  }

  // Menampilkan kriteria terbaik
  result += `<h3>Kriteria Terbaik</h3>`;
  result += `<p>Kriteria terbaik adalah <strong>K${
    bestCriteriaIndex + 1
  }</strong> dengan bobot ${highestWeight.toFixed(4)}</p>`;
